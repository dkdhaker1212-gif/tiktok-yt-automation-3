...
def build(video_path: str, out_jpg: str, title: str,
    ...
    try:
        ...
        if sr >= tr * 0.95:
            # wide (~16:9) source -> fill the whole thumbnail, centre-crop
            nh = THUMB_H
            nw = int(nh * sr)
            if nw < THUMB_W:
                nw, nh = THUMB_W, int(THUMB_W / sr)
            big = src.resize((nw, nh))
            canvas.paste(big, ((THUMB_W - nw) // 2, (THUMB_H - nh) // 2))
        else:
            # vertical source -> blurred cover + sharp centred portrait
            if sr > tr:
                nh = THUMB_H; nw = int(nh * sr)
            else:
                nw = THUMB_W; nh = int(nw / sr)
            cover = src.resize((nw, nh)).filter(ImageFilter.GaussianBlur(28))
